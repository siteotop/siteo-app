//import VeeEn from 'vee-validate/dist/locale/en';

export default {

    copy: 'Копировать', // in share element
    cancel: 'Отмена', // in share element
    close: 'Закрить', // in share element
    share: 'Репост', // in share element
    rm: 'Більше',
    //no_data: 'Данных для отображения нет',
    mainpage: 'Главная страница',
    open: 'Перейти',
    for_more: 'Для большей информации жми',
    here: 'здесь',
    top: 'ТОП',
    in: 'в',
    list: 'список',
    empty: 'пустой',
    /**
      common form
    */
    commonForm: {
      reset: 'Обновить форму',
      success: 'Успешный ответ',
      leave: 'Ви желаете прервать заполнение формы?',
      leave_desc: 'Все заполненные данные будут утеряны.',
      send: 'Отправить',
      error: {
         no_valid: 'Ошибки валидации'
      },
      captcha: {l: 'Каптча', d: 'Проверка роботом'}
    },

    //copyright:'Website created using "{template}" app and hosted on platform {url}',
    error404: 'Извините, но страница не найдена. Либо она не существует или была удалена. Нажмите кнопку ниже, чтобы вернуться на главную страницу',

    cookiePolicy: 'Мы используем файлы cookie, чтобы обеспечить вам лучший опыт на нашем сайте. Закрывая это сообщение, вы даете согласие на наши файлы cookie на этом устройстве соответствии с нашей политикой по cookie, если вы их не отключили.',
    cookieAct: 'Принять'
}
