<template>
  ...
  <v-layout>
      <v-flex xs4  v-for="(colorOptions, colorName ) in themeColorsKebab" :key="colorName"  >
          <v-card tile v-for="(shadowOptions, shadowName) in colorOptions.shades" :key="shadowName" :class="shadowOptions.value.name +' '+shadowOptions.value.text" >
            <v-card-text class="ma-0 pa-1 pt-3 pb-3" :style="{cursor:'pointer'}" @click="setNewActiveClass(shadowOptions.value)">
                <v-layout >
                    ...
                    <v-flex >
                      ...
                      <AppIcon name="check" scale="1" v-if="value==shadowOptions.value.name"></AppIcon>
                    </v-flex>
                </v-layout>
            </v-card-text>
          </v-card>
